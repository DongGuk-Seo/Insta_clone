...
def create_reply(request, post_id, comment_id, body:CommentSchema):
    comment = Comment.objects.get(id=comment_id)
    Reply = Reply(comment=comment,user=request.user,content=body.content)
    Reply.save()
    return {"detail" : "답글을 작성하셨습니다"}

@router.get('/{int:post_id}/{int:comment_id}', auth=JWTAuth())
...
def update_reply(request, post_id, comment_id, reply_id, body:CommentSchema):
...
def delete_reply(request, post_id, comment_id, reply_id):
    reply = Reply.objects.get(id=reply_id)
    if request.user == reply.user:
        reply.delete()
